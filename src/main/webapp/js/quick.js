...
addController(require("../js/ctrls/QuickNavCtrl.js"));
addController(require("../js/ctrls/QuickStepCtrl.js"));
addController(require("../js/ctrls/QuickPairCtrl.js"));
addController(require("../js/ctrls/QuickTransferCtrl.js"));
...
app.constant('iceServers', constants.iceServers)
    ...
    .config(/*@ngInject*/ ($compileProvider, aHrefSanitizationWhitelist) => {
        ...
        //add pair as the default route
        $routeProvider.otherwise({
            redirectTo: '/pair',
            activetab: 'pair'
        });
    }).run(/*@ngInject*/ ($rootScope, $location, connectUtils) => {

        //force a redirect to pair screen if not paired
        //todo check current location to determine if location change is needed
        //todo watch devices to check if it ever becomes empty
